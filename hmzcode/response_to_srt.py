...
class ResponseToSRT (threading.Thread):
    def __init__(self, q):
        ...
        self.threadID = G_SRT_THREAD_ID
        self.name = G_SRT_THREAD_NAME
        self.q = q
        self.exit_flag = False
        self.verbose_mode = G_VERBOSE
        self.logger = amg_logger.amagi_logger (
                        "com.amagi.stt.resp_to_srt", 
                        amg_logger.LOG_INFO, 
                        log_stream=G.G_LOGGER_STREAM)
        if G_OFLAGS_APPEND_MODE:
            self.open_mode = "a"
        else:
            self.open_mode = "w"
        if G_OFLAGS_APPEND_NULL_CHAR:
            self.null_char = "\0"
        else:
            self.null_char = ""
        self.last_rcvd_epoch = 0
        self.srt_count = 0
        self.srt_fname = G_OUTPUT_SRT_PATH
    ...
    def srt_writer (self, text, tc_in, duration_ms):
        with open (self.srt_fname, self.open_mode) as fp:
            ...
            if False:#len(self.last_srt_sub) > 0 and (tc_in-self.last_srt_sub[1] < G.G_REPEAT_SUB_THREASHOLD_MS):
    ...
    def get_duration (self, wr_list):
        ...
        duration = duration if duration > 0.9 else G.G_MAX_INTER_WORD_DURATION
        return duration
    
    def can_consume_subtitle (self, time_now):

        if len (self.words_list) == 0 or \
          self.words_list[-1][G.G_RESP_LIST_WORD_CNSMD_INDX]:
            # last word is consumed.
            return False

        min_timeout_since_last_word = max (1.5, \
                G.G_MIN_WORD_DRAIN_DELAY - G.G_MAX_SUBTITLE_LINE_DURATION/1000.0)
        last_word_time = self.words_list[-1][G.G_RESP_LIST_WORD_MTIME_INDX]
        for it in self.words_list:
            if not it[G.G_RESP_LIST_WORD_CNSMD_INDX]:
                # if last-word-time - fist-unconsumed-word-time > Threshold or
                # if curr-sys-time - first-unconsumed-word-time > Threshold
                # then consumed 'em.
                if last_word_time - it[G.G_RESP_LIST_WORD_MTIME_INDX] >= (1000.0*G.G_MIN_WORD_DRAIN_DELAY) or \
                    time_now - it[G.G_RESP_LIST_WORD_TIME_INDX] >= min_timeout_since_last_word:
                    self.logger.debug ("mtime diff= %d > global:%d, epoch_diff = %d" %(\
                            last_word_time - it[G.G_RESP_LIST_WORD_MTIME_INDX],\
                            G.G_MIN_WORD_DRAIN_DELAY, \
                            time_now - it[G.G_RESP_LIST_WORD_TIME_INDX]))
                    return True
        return False
    ...
    def write (self):
        ...
        for it in self.words_list:
            if not it[G.G_RESP_LIST_WORD_CNSMD_INDX]:
                ...
                if (len (text.strip()) + len (it[G.G_RESP_LIST_WORD_INDX]) + 1) > \
                    G.G_MAX_CHARS_IN_SUB_ROW:
                    # Case where single word len > line-length.
                    if len (text) == 0:
                        self.logger.warn ("Chopping off word larger than 32 charaters !! - word=" \
                                + it[G.G_RESP_LIST_WORD_INDX].encode('utf-8'))
                        it[G.G_RESP_LIST_WORD_CNSMD_INDX] = True
                        self.last_consumed_pts = it[G.G_RESP_LIST_WORD_MTIME_INDX]
                        text = text + it[G.G_RESP_LIST_WORD_INDX] + " "
                        text = text[:G.G_MAX_CHARS_IN_SUB_ROW]
                        wr_list.append (it)
                    # write the SRT.
                    duration_ms = self.get_duration ([i[G.G_RESP_LIST_WORD_MTIME_INDX] for i in wr_list])
                    self.srt_writer (text.strip(), wr_list[0][G.G_RESP_LIST_WORD_MTIME_INDX], duration_ms)
                    return
                else:
                    # Accumulate the line.
                    if len(wr_list) > 0 and \
                       (it[G.G_RESP_LIST_WORD_MTIME_INDX] - \
                          wr_list[0][G.G_RESP_LIST_WORD_MTIME_INDX] > \
                          G.G_MAX_SUBTITLE_LINE_DURATION or \
                        it[G.G_RESP_LIST_WORD_MTIME_INDX] - \
                          wr_list[-1][G.G_RESP_LIST_WORD_MTIME_INDX] > \
                          G.G_MAX_INTER_WORD_DURATION):
                            break
                    wr_list.append (it)
                    text = text + it[G.G_RESP_LIST_WORD_INDX] + " "
                    it[G.G_RESP_LIST_WORD_CNSMD_INDX] = True
    ...
    def response_to_word_time_offset (self, response, rtime, mapped_time):
        ...
        l_max_words_to_search = G.G_MAX_WORDS_TO_SEARCH
        ...
        for word in new_response:
            ...
            while (local_pivot < rl_l) and local_pivot - pivot_in_old_list < l_max_words_to_search:
                if word.upper().strip(".").strip(",") == \
                   old_list[local_pivot][G.G_RESP_LIST_WORD_INDX].upper().strip(".").strip(","):
                    is_word_old.append (True)
                    '''
                    if local_pivot == pivot_in_old_list:#i=0
                        # if match is found then try to set search words to G.G_MAX_WORDS_TO_SEARCH.
                        l_max_words_to_search -= 1
                        l_max_words_to_search = max (G.G_MAX_WORDS_TO_SEARCH, l_max_words_to_search)
                    '''
                    ...
                    self.logger.debug ("lp=%d, rl_l=%d, word=%s, old=%s" %(local_pivot, rl_l, word, \
                        old_list[local_pivot][G.G_RESP_LIST_WORD_INDX].upper()))
                    local_pivot += 1
        ...
        '''
        if (rl_l > 0 and not old_list[0][G.G_RESP_LIST_WORD_CNSMD_INDX] and not is_word_old[0]) or \
            len(is_word_old) == 0:
            #print ("--------------------------")
        ...
        i = 0
        old_rtime = old_list[i][G.G_RESP_LIST_WORD_TIME_INDX]
        old_mtime = old_list[i][G.G_RESP_LIST_WORD_MTIME_INDX]
        prev_mtime = 0
        ...
        while i >= 0:
            if is_word_old[i]:
                ...
                self.words_list[i][G.G_RESP_LIST_WORD_TIME_INDX] = rtime
                ...
                self.words_list[i][G.G_RESP_LIST_WORD_MTIME_INDX] = mapped_time + i/1000.0
                self.words_list[i][G.G_RESP_LIST_WORD_CNSMD_INDX] = False
            i -= 1
        ...
        while not self.exit_flag:
            ...
            try:
                if result:
                    ...
                    cur_timestamp = (stream.restart_counter * G.G_STREAMING_LIMIT) + \
                ...
                if result and result.is_final:
                    ...
                    while len(self.words_list) > 0 and \
                        (not self.words_list[-1][G.G_RESP_LIST_WORD_CNSMD_INDX]):
                        self.write ()
